...
public class IncomeUpdatedMessageConsumer : BaseConsumer<IncomeUpdatedMessage>
{
	public IncomeUpdatedMessageConsumer(AppDbContext dbContext, IDeduplicationService deduplicationService) : base(dbContext, deduplicationService)
	{
	}
	...
	protected override async Task ProcessMessage(IncomeUpdatedMessage message)
	{
		...
		var expenseEvent = new ExpenseServiceIncomeEvent(message.EventId, "Update", DateTime.UtcNow, message.UserId, eventData);
